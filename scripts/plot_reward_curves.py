...
RUN2_CSV = (
    REPO_DIR
    / "ray_results"
    / "PPO_option2_selfplay_dense_cpu_slurm"
    / "PPO_Soccer_ba9af_00000_0_2026-04-23_19-39-54"
    / "progress.csv"
)
OUT_SVG = REPO_DIR / "report_artifacts" / "reward_curve_submission2_policy_default.svg"
OUT_PNG = REPO_DIR / "report_artifacts" / "reward_curve_submission2_policy_default.png"
...
def rolling_mean(points, window: int):
    ...
    for i in range(window - 1, len(points)):
        out.append((points[i][0], statistics.mean(v for _, v in points[i - window + 1 : i + 1])))
    return out
...
def main():
    raw = load_series(RUN2_CSV, "policy_reward_mean/default")
    smooth = rolling_mean(raw, 50)

    if not raw:
        raise RuntimeError(f"No data loaded from: {RUN2_CSV}")
    ...
    smooth_scaled = scale_points(smooth, left, top, plot_w, plot_h, x_min, x_max, y_min, y_max)

    x_ticks_m = [0, 0.5, 1, 2, 3, 4, 4.3]
    ...
    parts.append(
        ...
        'Reward Curve for REINFORCE_AGENT (Reward Shaping + Self-Play)'
        "</text>"
    )
    parts.append(
        '<text x="110" y="92" font-family="Arial" font-size="15" fill="#444">'
        'Metric: policy_reward_mean/default'
        "</text>"
    )
    ...
    parts.append(polyline(raw_scaled, "#9fcdb5", width=1.5, opacity=0.9))
    parts.append(polyline(smooth_scaled, "#1d7f49", width=3.0))
    ...
    parts.append(
        ...
        '<text text-anchor="middle" font-family="Arial" font-size="14">policy_reward_mean/default</text>'
        "</g>"
    )

    legend_x, legend_y = left + plot_w - 330, top + 18
    parts.append(
        f'<rect x="{legend_x}" y="{legend_y}" width="305" height="70" fill="#fafafa" stroke="#dddddd" />'
    )
    parts.append(
        f'<line x1="{legend_x + 12}" y1="{legend_y + 22}" x2="{legend_x + 52}" y2="{legend_y + 22}" stroke="#9fcdb5" stroke-width="1.5" />'
    )
    parts.append(
        f'<text x="{legend_x + 60}" y="{legend_y + 26}" font-family="Arial" font-size="12">Raw policy_reward_mean/default</text>'
    )
    parts.append(
        f'<line x1="{legend_x + 12}" y1="{legend_y + 48}" x2="{legend_x + 52}" y2="{legend_y + 48}" stroke="#1d7f49" stroke-width="3" />'
    )
    parts.append(
        f'<text x="{legend_x + 60}" y="{legend_y + 52}" font-family="Arial" font-size="12">Rolling mean (window=50)</text>'
    )
